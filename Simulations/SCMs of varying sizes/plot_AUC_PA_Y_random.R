#### Plots for random graph with random parents - using AUC on models of sizes 1, 2 and 3

setwd("~/Dropbox/KU/Thesis/R")
library(ggplot2)

res_ell_2 <- readRDS(file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents.RDS")
res_ell_1 <- readRDS(file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_ell_1.RDS")
res_reg <- readRDS(file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_reg.RDS")
...
ggsave(filename = "Graphics/AUC_PA_Y_random_all_models.pdf", plot = g, width = 5.8, height = 3)
...
res_ell_2 <- readRDS(file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_env_all.RDS")
res_ell_1 <- readRDS(file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_ell_1_env_all.RDS")
res_reg <- readRDS(file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_reg_env_all.RDS")
...
ggplot(data = tmp, aes(x = Frequency_of_H, y = Average_AUC, fill = Exp, color = Exp)) + 
  geom_point(size = 3)  + 
  ggtitle("Average AUC with randomized graph and random parents of Y, all models") + 
  xlab("Frequency of H") + 
  ylab("Average AUC") + 
  theme_minimal() + 
