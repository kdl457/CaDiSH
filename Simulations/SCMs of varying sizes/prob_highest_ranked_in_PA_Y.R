#### using models of size 1
#### What is probability that the highest ranked variable is a parent of Y?
...
omega_H <- seq(from = 0, to = 40, length.out = 5)
B <- 2 # Number of simulations for each value of omega
n_CI <- 2 # Number of repetitions used for confidence intervals

parent_res <- c() # Is the correct parent recovered?
parent_average <- c() # Probability of recovering correct parent
parent_res_ell_1 <- c()
parent_average_ell_1 <- c()
parent_res_reg <- c()
parent_average_reg <- c()
res_ran_parents_1_new <- matrix(NA, nrow = n_CI, ncol = length(omega_H))
res_ran_parents_1_ell_1_new <- matrix(NA, nrow = n_CI, ncol = length(omega_H))
res_ran_parents_1_reg_new <- matrix(NA, nrow = n_CI, ncol = length(omega_H))
...
#      CH_E <- as.numeric(sample(c(1:7)[c(1:7) != Y_rank], size = sample(1:6, size = 1))) # Which variables are affected by E.
      CH_E <- as.numeric(c(1:7)[c(1:7) != Y_rank])
      parent_list <- list()
      ...
      # Is highest ranked a parent?
      
      parent_res <- ranking_vars[1] %in% parent_list[[Y_rank]]
      ...
      parent_res_ell_1 <- ranking_vars[[1]] %in% parent_list[[Y_rank]]
      ...
      models_2 <- combn(c(1:7)[c(1:7) != Y_rank], m = 2) # All possible models of size 1
      scores_2 <- c()  
      
      for (i in 1:ncol(models_2)) {
        scores_2[i] <- summary(lm(X_list[[Y_rank]] ~ X_list[[models_2[,i][1]]]))$fstatistic[1]
      }
      
      models_3 <- combn(c(1:7)[c(1:7) != Y_rank], m = 3) # All possible models of size 1
      scores_3 <- c()  
      
      for (i in 1:ncol(models_3)) {
        scores_3[i] <- summary(lm(X_list[[Y_rank]] ~ X_list[[models_3[,i][1]]]))$fstatistic[1]
      }
      ...
      parent_res_reg <- ranking_vars[[1]] %in% parent_list[[Y_rank]]
      
    }
    parent_average[h] <- sum(parent_res) / B
    parent_average_ell_1[h] <- sum(parent_res_ell_1) / B
    parent_average_reg[h] <- sum(parent_res_reg) / B
  }
  res_ran_parents_1_new[,om_H] <- parent_average
  res_ran_parents_1_ell_1_new[,om_H] <- parent_average_ell_1
  res_ran_parents_1_reg_new[,om_H] <- parent_average_reg
}
res_ran_parents_1_new
res_ran_parents_1_ell_1_new
res_ran_parents_1_reg_new

saveRDS(object = res_ran_parents_1_new, file = "Simulations/Causal Discovery/Randomized Graph/res_ran_parents_1_new.RDS")
saveRDS(object = res_ran_parents_1_ell_1_new, file = "Simulations/Causal Discovery/Randomized Graph/res_ran_parents_1_ell_1_new.RDS")
saveRDS(object = res_ran_parents_1_reg_new, file = "Simulations/Causal Discovery/Randomized Graph/res_ran_parents_1_reg_new.RDS")
