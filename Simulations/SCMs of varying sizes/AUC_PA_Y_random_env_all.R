#### Using AUC. Letting environments affect all variables but Y.
...
AUC_res_reg <- c()
AUC_average_reg <- c()
res_AUC_ran_parents_env_all <- matrix(NA, nrow = n_CI, ncol = length(omega_H))
res_AUC_ran_parents_ell_1_env_all <- matrix(NA, nrow = n_CI, ncol = length(omega_H))
res_AUC_ran_parents_reg_env_all <- matrix(NA, nrow = n_CI, ncol = length(omega_H))
...
for (om_H in 1:length(omega_H)) {
  ...
  for (h in 1:n_CI) {
    ...
    for (b in 1:B) {
      ...
      CH_E <- as.numeric(c(1:7)[c(1:7) != Y_rank]) # All variables but Y are affected by E.
      ...
      F_count <- 0
      for (i in 1:6) {
        AUC <- (ranking_vars[i] %in% parent_list[[Y_rank]]) * (6 - length(parent_list[[Y_rank]]) - F_count) + AUC
  ...
  res_AUC_ran_parents_env_all[,om_H] <- AUC_average
  res_AUC_ran_parents_ell_1_env_all[,om_H] <- AUC_average_ell_1
  res_AUC_ran_parents_reg_env_all[,om_H] <- AUC_average_reg
}
res_AUC_ran_parents_env_all
res_AUC_ran_parents_ell_1_env_all
res_AUC_ran_parents_reg_env_all

saveRDS(object = res_AUC_ran_parents_env_all, file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_env_all.RDS")
saveRDS(object = res_AUC_ran_parents_ell_1_env_all, file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_ell_1_env_all.RDS")
saveRDS(object = res_AUC_ran_parents_reg_env_all, file = "Simulations/Causal Discovery/Randomized Graph/res_AUC_ran_parents_reg_env_all.RDS")
