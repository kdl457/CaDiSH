##### Plot 2: Varying the influence of H
...
lambda <- 1.2 # Fixed lambda. LOOCV could be used but takes way too long. Maybe do it for the first.
...
res_varying_influence_50 <- matrix(NA, nrow = n_CI, ncol = length(h_y_vec))
...
for (h_y in 1:length(h_y_vec)) {
  ...
  res_varying_influence_50[,h_y] <- CD_accuracy
}

res_varying_influence_50
saveRDS(object = res_varying_influence_50, file = "Simulations/Causal Discovery/Plot2/res_varying_influence_50.RDS")
...
B <- 50 # Number of simulations for each specific alpha
n_CI <- 50 # Number of repetitions of simulations for each specific alpha
lambda <- 0.1 # Fixed lambda. LOOCV could be used but takes way too long. Maybe do it for the first.
n_sim <- 50 # Number of observations from each SCM
h_y_vec <- seq(0, 20, length.out = 11) # Values of h_y - large h_y = large effect of H on beta_2 in Y
res_varying_influence_50_ell1 <- matrix(NA, nrow = n_CI, ncol = length(h_y_vec))
...
for (h_y in 1:length(h_y_vec)) {
  ...
  res_varying_influence_50_ell1[,h_y] <- CD_accuracy
}

res_varying_influence_50_ell1
saveRDS(object = res_varying_influence_50_ell1, file = "Simulations/Causal Discovery/Plot2/res_varying_influence_50_ell1.RDS")
...
n_sim <- 50 # Number of observations from each SCM
h_y_vec <- seq(0, 20, length.out = 11) # Values of h_y - large h_y = large effect of H on beta_2 in Y
res_varying_influence_50_10pi <- matrix(NA, nrow = n_CI, ncol = length(h_y_vec))
...
for (h_y in 1:length(h_y_vec)) {
  ...
  for (j in 1:n_CI) {
    ...
    for (i in 1:B) {
      cat(paste("h_y", h_y_vec[h_y], "Repetition", j, "Number",i))
      data <- sim_data_fixed_env_2(sim_length = n_sim, breaks = c(0, 1/4, 1/2, 3/4, 1), h_func = f_hidden_sin_10pi, 
                                   alpha_3 = coeff_var[17*(i-1) + 1], h_3 = h_var[4*(i-1) + 1], n_3 = noise_var[8*(i-1) + 1], 
                                   alpha_2 = coeff_var[(17*(i-1) + 2):(17*(i-1) + 4)], 
                                   c_23 = coeff_var[(17*(i-1) + 5):(17*(i-1) + 7)], h_2 = h_var[(4*(i-1) + 2):(4*(i-1) + 4)], 
                                   n_2 = noise_var[(8*(i-1) + 2):(8*(i-1) + 4)], alpha_y = coeff_var[17*(i-1) + 8], 
  ...
  res_varying_influence_50_10pi[,h_y] <- CD_accuracy
}

res_varying_influence_50_10pi
saveRDS(object = res_varying_influence_50_10pi, file = "Simulations/Causal Discovery/Plot2/res_varying_influence_50_10pi.RDS")
...
h_y_vec <- seq(0, 20, length.out = 11) # Values of h_y - large h_y = large effect of H on beta_2 in Y
res_varying_influence_200 <- matrix(NA, nrow = n_CI, ncol = length(h_y_vec))
...
for (h_y in 1:length(h_y_vec)) {
  ...
  res_varying_influence_200[,h_y] <- CD_accuracy
}

res_varying_influence_200
saveRDS(object = res_varying_influence_200, file = "Simulations/Causal Discovery/Plot2/res_varying_influence_200.RDS")
...
n_sim <- 50 # Number of observations from each SCM
h_y_vec <- seq(0, 20, length.out = 11) # Values of h_y - large h_y = large effect of H on beta_2 in Y
res_varying_influence_50_reg <- matrix(NA, nrow = n_CI, ncol = length(h_y_vec))
...
for (h_y in 1:length(h_y_vec)) {
  ...
  res_varying_influence_50_reg[,h_y] <- CD_accuracy
}

res_varying_influence_50_reg
saveRDS(object = res_varying_influence_50_reg, file = "Simulations/Causal Discovery/Plot2/res_varying_influence_50_reg.RDS")
